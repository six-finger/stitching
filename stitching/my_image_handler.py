...
class ImageHandler:
    ...
    def __init__(
        ...
        self.incre_img = None       # Add 20230719
        self.incre_mask = None       # Add 20230719
        

    def set_img_names(self, img_names):
        if len(img_names) == 1:
            img_names = glob.glob(img_names[0])
        if len(img_names) < 2 and self.incre_img is None:   # Add 20230719
            raise StitchingError("2 or more Images needed for Stitching")
        self.img_names = img_names

    def set_incre_img(self, incre_img):     # Add 20230719
        self.incre_img = incre_img

    def set_incre_mask(self, incre_mask):     # Add 20230719
        self.incre_mask = incre_mask

    # ===== Modify 230719 below ===== 
    def resize_to_medium_resolution(self, img_lst, img_names):
        return self.read_and_resize_imgs(self.medium_scaler, img_lst=img_lst, img_names=img_names)
    ...
    def resize_to_final_resolution(self, img_lst, img_names):
        return self.read_and_resize_imgs(self.final_scaler, img_lst=img_lst, img_names=img_names)

    def read_and_resize_imgs(self, scaler, img_lst=None, img_names=None):
        for img, size in self.input_images(img_lst, img_names):
            yield self.resize_img_by_scaler(scaler, size, img)
        if self.incre_img is not None:
            for img, size in self.input_images(img_lst=[self.incre_img], img_names=None, init=False):
                yield self.resize_img_by_scaler(scaler, size, img)


    # ===== Add 230719 below ===== 
    def mask_resize_to_medium_resolution(self, img_lst, img_names):
        return self.mask_read_and_resize_imgs(self.medium_scaler, img_lst=img_lst, img_names=img_names)

    def mask_resize_to_low_resolution(self, medium_imgs=None):
        if medium_imgs and self.scales_set:
            return self.resize_imgs_by_scaler(medium_imgs, self.low_scaler)
        return self.mask_read_and_resize_imgs(self.low_scaler)

    def mask_resize_to_final_resolution(self, img_lst, img_names):
        return self.mask_read_and_resize_imgs(self.final_scaler, img_lst=img_lst, img_names=img_names)

    def mask_read_and_resize_imgs(self, scaler, img_lst=None, img_names=None):
        for img, size in self.input_images(img_lst, img_names):
            yield self.resize_img_by_scaler(scaler, size, img)
        if self.incre_mask is not None:
            for img, size in self.input_images(img_lst=[self.incre_mask], img_names=None, init=False):
                yield self.resize_img_by_scaler(scaler, size, img)

    # ===== Modified 230719 below =====
    def input_images(self, img_lst, img_names, init=True):
        if init:
            self.img_sizes = []
